...
function scoreSpatialMemory(answer, expected = []) {
  const text = String(answer || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  let hits = 0;
  expected.forEach((item) => {
    const parts = item.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").split(" ");
    const ok = parts.filter((p) => p.length > 2).some((p) => text.includes(p));
    if (ok) hits += 1;
  });
  const score = expected.length ? Math.round((hits / expected.length) * 100) : 0;
  return {
    score,
    maxScore: 100,
    feedback: score >= 80 ? "Buen recuerdo espacial." : score >= 50 ? "Recuerdo espacial parcial." : "Conviene reforzar ubicación y asociación espacial.",
...
function VisualAttentionModule() {
  return <GenericOpenModule
    module="atencion_visual"
    title="Atención visual funcional"
    subtitle="Ejercicios con estímulos visuales autogenerados: objetos, señales, compra y vida diaria."
    icon={Image}
    tasks={visualAttentionTasks}
    pillClass="attention"
    scorer={(task, answers) => attentionScore(answers.respuesta, task.answer)}
    renderBody={({ task, answers, setAnswers }) => (
      <>
        <p><b>{task.instructions}</b></p>
        <p>Objetivo: <span className="pill attention">{task.target}</span></p>
        <div className="card" style={{ background: "#f8fafc" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: "12px" }}>
            {task.icons.map((icon, index) => (
              <motion.div key={index} className="mini-card" whileHover={{ scale: 1.05 }} style={{ fontSize: "2rem", textAlign: "center" }}>
                {icon}
              </motion.div>
            ))}
          </div>
        </div>
        <label style={{ marginTop: 16 }}><span>Respuesta</span><input value={answers.respuesta || ""} onChange={(e) => setAnswers({ ...answers, respuesta: e.target.value })} placeholder="Número correcto"/></label>
      </>
    )}
  />;
}

function SpatialMemoryModule() {
  const [visible, setVisible] = useState(true);
  return <GenericOpenModule
    module="memoria_espacial"
    title="Memoria espacial"
    subtitle="Recordar ubicaciones, objetos y distribución de espacios cotidianos."
    icon={Grid3X3}
    tasks={spatialMemoryTasks}
    pillClass="memory"
    scorer={(task, answers) => scoreSpatialMemory(answers.recuerdo || "", task.expected)}
    renderBody={({ task, answers, setAnswers }) => (
      <>
        <p>{task.prompt}</p>
        <Button variant="outline" onClick={() => setVisible(!visible)}>{visible ? <EyeOff size={16}/> : <Eye size={16}/>} {visible ? "Ocultar cuadrícula" : "Ver cuadrícula"}</Button>
        {visible && (
          <div className="card" style={{ marginTop: 14, background: "#f8fafc" }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "10px" }}>
              {task.grid.flat().map((cell, index) => (
                <div key={index} className="mini-card" style={{ textAlign: "center", fontSize: cell.length <= 2 ? "2rem" : "1rem", minHeight: "70px", display: "grid", placeItems: "center" }}>{cell}</div>
              ))}
            </div>
          </div>
        )}
        <InputArea label="Escribe qué recuerdas y dónde estaba ubicado" value={answers.recuerdo || ""} onChange={(v) => setAnswers({ ...answers, recuerdo: v })}/>
      </>
    )}
  />;
}
...
function PrivacyNotice() {
  return (
    <section className="section">
      <SectionTitle icon={ShieldCheck} title="Uso seguro" subtitle="Recomendaciones para usar el portal con pacientes sin exponer datos sensibles." />
      <div className="grid-2">
        <div className="card"><h3>Códigos de paciente</h3><p>Usa códigos como PAC-001 o M01. Evita nombres completos, diagnósticos o información clínica sensible.</p></div>
        <div className="card"><h3>Google Sheets</h3><p>El registro central está en tu hoja. Revisa permisos y comparte solo contigo o con quien corresponda.</p></div>
      </div>
    </section>
  );
}
  ...
  return (
    <section className="section">
      ...
      <div className="grid-2">
        ...
        <div className="card">
          <h3>Comparación entre pacientes</h3><p><b>Eje X:</b> código de paciente. <b>Eje Y:</b> promedio global de rendimiento en porcentaje.</p><p><b>Eje X:</b> código de paciente. <b>Eje Y:</b> promedio global de rendimiento en porcentaje.</p>
          {comparison.length ? <ResponsiveContainer width="100%" height={240}><BarChart data={comparison}><XAxis dataKey="patient"/><YAxis domain={[0,100]}/><Tooltip/><Bar dataKey="promedio" fill="#4f46e5"/></BarChart></ResponsiveContainer> : <p>Aún no hay datos de pacientes.</p>}
        </div>
      </div>

      <div className="grid-2" style={{ marginTop: 18 }}>
        <div className="card">
          <h3>Promedio por módulo</h3><p><b>Eje X:</b> área cognitiva. <b>Eje Y:</b> rendimiento medio de 0 a 100.</p><p><b>Eje X:</b> área cognitiva. <b>Eje Y:</b> rendimiento medio de 0 a 100.</p>
          {moduleStats.length ? <ResponsiveContainer width="100%" height={240}><BarChart data={moduleStats}><XAxis dataKey="module"/><YAxis domain={[0,100]}/><Tooltip/><Bar dataKey="porcentaje" fill="#059669"/></BarChart></ResponsiveContainer> : <p>Aún no hay puntuaciones suficientes.</p>}
        </div>
        <div className="card">
          <h3>Evolución individual</h3><p><b>Eje X:</b> número de sesión. <b>Eje Y:</b> puntuación obtenida en porcentaje.</p><p><b>Eje X:</b> número de sesión. <b>Eje Y:</b> puntuación obtenida en porcentaje.</p>
          {timeline.length ? <ResponsiveContainer width="100%" height={240}><LineChart data={timeline}><XAxis dataKey="name"/><YAxis domain={[0,100]}/><Tooltip/><Line type="monotone" dataKey="porcentaje" stroke="#dc2626" strokeWidth={3}/></LineChart></ResponsiveContainer> : <p>Aún no hay puntuaciones suficientes.</p>}
        </div>
      </div>

      <div className="grid-2" style={{ marginTop: 18 }}>
        <div className="card">
          <h3>Perfil cognitivo</h3><p>Mapa visual de fortalezas y áreas a reforzar por dominio cognitivo.</p><p>Mapa visual de fortalezas y áreas a reforzar por dominio cognitivo.</p>
          {radarData.some(d => d.value > 0) ? <ResponsiveContainer width="100%" height={260}><RadarChart data={radarData}><PolarGrid/><PolarAngleAxis dataKey="module"/><PolarRadiusAxis domain={[0,100]}/><Radar dataKey="value" stroke="#4f46e5" fill="#4f46e5" fillOpacity={0.35}/></RadarChart></ResponsiveContainer> : <p>Aún no hay perfil suficiente.</p>}
        </div>
        <div className="card">
  ...
  const generateWithAI = async (type) => {
    ...
    try {
      ...
      if (data.ok && data.exercise) {
        setGenerated({ type, task: data.exercise });
        setAnswers({});
        setLastResult(null);
        setStartTime(Date.now());
        setStatus("Ejercicio generado con IA.");
      } else {
        setStatus(data.message || "IA no disponible. Usa los generadores gratuitos.");
      }
    } catch {
      setStatus("No se pudo generar con IA. Usa los generadores gratuitos.");
    }
  };
    ...
    <section className="section">
      ...
      {generated && <div className="card" style={{ marginTop: 18 }}>
        ...
        {generated.type === "memoria" && <>
          ...
          <InputArea label={task.type === "ruta" ? "Escribe la ruta completa en orden, con lugar y tarea" : "Escribe lo que recuerdas"} value={answers.recuerdo || ""} onChange={(v) => setAnswers({ ...answers, recuerdo: v })}/>
        </>}
